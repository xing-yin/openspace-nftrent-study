...
import { RFTToken } from "./RFTToken.sol";

contract RFTStake {
  RFTToken public immutable rnt;

  EsRNTToken public immutable esRnt;

  constructor(address _rntCa, address _esRntCa) {
    rnt = RFTToken(_rntCa);
    esRnt = EsRNTToken(_esRntCa);
  }
  ...
  function queryStakeInfo(address user) external view returns (StkInfoForView memory info) {
    StkInfo storage s = stakes[user];
    info = StkInfoForView(s.amount, s.esRntReward + algReward(s.amount, s.lastUpdate));
  }
  ...
  function algReward(uint256 amount, uint256 lastUpdate) internal view returns (uint256) {
    return (amount * (block.timestamp - lastUpdate)) / (24 * 3600);
  }

  // 计算解锁量。esRNT 是锁仓性的 RNT， 1 esRNT 在 30 天后可兑换 1 RNT，随时间线性释放
  function algUnlock(uint256 amount, uint256 lastUpdate) internal view returns (uint256) {
  ...
  function updateLock(uint256 _lockedAmount) internal {
    if (locks[msg.sender].lastUpdate == 0) {
      ...
      uint256 newUnlock = algUnlock(lk.lockedAmount - lk.unlockAmount, lk.lastUpdate);
  ...
  // 查询锁仓信息
  function queryLockInfo(address user) external view returns (LockInfoForView memory info) {
  ...
  function exchangeEsRnt(bool unlockBurn) external {
    ...
    emit Exchange(msg.sender, lk.lockedAmount, lk.unlockAmount, lk.burnAmount);
