        ...
        uint256(
          keccak256(
            abi.encodePacked(
              ...
              factory,
              keccak256(abi.encodePacked(token0, token1)),
              hex"3de92edd16b0417abc5cf4a4035f763e05bef4ad85b5b71ad6cb9480dd81a491" // init code hash: this is UniswapV2Pair creationCode ==> keccak256(abi.encodePacked(type(UniswapV2Pair).creationCode))
            )
          )
        )
      )
...
contract UniswapV2Router01 is IUniswapV2Router01 {
   ...
   * @param amountAMin 最小数量A
   * @param amountBMin 最小数量B
   * @param to to地址（即配对合约中铸造函数中的to地址）
   * @param deadline 最后期限
   * @return amountA   数量A
   * @return amountB   数量B
    ...
    returns (uint256[] memory amounts)
  {
    ...
    console.log("swapExactETHForTokens amounts:", amounts[0]);
    //确认数额数组最后一个元素>=最小输出数额
    require(amounts[amounts.length - 1] >= amountOutMin, "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT");
